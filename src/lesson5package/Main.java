...
//        Для второго метода замеряете время разбивки массива на 2, просчета каждого из двух массивов и склейки.
        ...
        for(float f : oneFloatArray) { f = 1.0f;}
        System.arraycopy(oneFloatArray, 0, multyFloatArray, 0, size);

        long a = currentTimeMillis();
        System.out.println("Работа в один поток = " + (oneFloatMethod(oneFloatArray) - a) + " милисекунд");

        a = currentTimeMillis();
        System.out.println("Работа в два потока = " + (multyFloatMethod(multyFloatArray) - a) + " милисекунд");


    }

    private static long oneFloatMethod(float[] arr){
        for (int i = 0; i < size; i++) {
            arr[i] = (float)(arr[i] * Math.sin(0.2f + i / 5) * Math.cos(0.2f + i / 5) * Math.cos(0.4f + i / 2));
        }
    ...
    private static long multyFloatMethod(float[] arr){
